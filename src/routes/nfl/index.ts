...
const example: FastifyPluginAsync = async (fastify: any): Promise<void> => {
  ...
  fastify
    .get('/headshots/gameResult', async function (request: FastifyRequest, reply: FastifyReply) {
...
const getItemsAndTotalCountWithPagination = async (fastify: any, limit: number, offset: number): Promise<Records> => {
...
const findById = async (fastify: any, id: string) => {
  return await fastify.mongo.db.collection('nfl-headshot').findOne({ _id: new ObjectId(id) });
}
